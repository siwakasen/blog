---
title: Set Up a Linux Hypervisor with KVM, QEMU, and libvirt
description:
  Turn an old laptop or server into a Linux hypervisor with KVM, QEMU, and libvirt, then create and manage virtual machines from the command line.
date: '2025-12-21T08:49:06.000Z'
ogImage:
  title: Linux Hypervisor with KVM, QEMU, and libvirt
readingTime: 15 min read
tags: ['tech','linux','server','homelab']
---

{/* !start-of-preview */}

With Linux, you can combine different tools to create powerful setups. One example is provisioning VMs, and you can turn an old or unused laptop or server into a hypervisor without spending money on cloud services.
...
I stick to a stack that’s simple and reliable enough to get my work done.
...
```
  ...
  <Panel type="warning">
  ⚠️ Please note that package may vary depending on your Linux distribution. Always refer to your distribution's documentation for specific instructions. This guide was using Ubuntu 24.04 LTS for the hypervisor.
  </Panel>
  ...
  <div className="relative flex place-content-center h-80 not-prose">
    <Image
      ...
      fill
      alt="kvm tool"
      src={'/assets/blog/linux-hypervisor-setup/kvm-tools.avif'}
    />
  </div>
...
When using qemu:///system, access is controlled by polkit. There are a few easy ways to allow access to libvirtd.
- Add your user to the libvirt group. This group exists in newer libvirt versions and allows direct access. 
- Be part of an admin group. You will be asked for your sudo password when using virt-manager or virsh.
- Allow a group in polkit configuration. You can explicitly allow a group in polkit. With this setup, you can use virt-manager or virsh without being asked for a password.
- Edit <code>/etc/polkit-1/rules.d/50-libvirt.rules</code>
...
## Configuring libvirtd and Preparing ISOs
  ...
  To begin interacting with qemu/kvm you need to start the libvirt daemon first, but usually it will start automatically.
  ```bash
...
virt-install \
  ...
  --cdrom /var/lib/libvirt/isos/ubuntu-24.04.3-live-server-amd64.iso \
  --graphics vnc \
...
virt-install \
  ...
  --ram 2048 \
  --vcpus 2 \
  --disk path=/var/lib/libvirt/images/node-2.qcow2,size=60,format=qcow2,bus=virtio \
  --os-variant rocky-unknown \
  --cdrom /var/lib/libvirt/isos/Rocky-10.1-x86_64-boot.iso \
  --graphics vnc \
  --network network=default 
```
...
- `--graphics`: Set to VNC to access the VM's graphical console during initial installation.
  ...
  <div className="relative flex place-content-center h-40 not-prose">
    <Image
      className="object-contain"
      fill
      src={'/assets/blog/linux-hypervisor-setup/tigervnc.avif'}
      alt="TigerVNC"
    />
  </div>
...
```
  ...
  <div className="relative flex place-content-center h-40 not-prose">
    <Image
      className="object-contain"
      fill
      src={'/assets/blog/linux-hypervisor-setup/vm-list.avif'}
      alt="vm list"
    />
  </div>
...
```
  ...
  ```
  Then, we need to power off the VM:
  ```bash
  ...
  virsh start node-2
  ```
  Then, let's check the memory allocation again.
  ```bash
  virsh dominfo node-2
  # or check inside the vm using:
  free -h
  ```
## Wrapping Up
...
- [Build a highly available K3s cluster on top of this homelab setup](/posts/k3s-tailscale-cloudflare)
